...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remover_item'])) {
    ...
    $_SESSION['carrinho'] = array_values($_SESSION['carrinho']); // Reindexar o array após a remoção

    // Adicione o número da mesa como parâmetro na URL de redirecionamento
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['observacao'])) {
    foreach ($_POST['observacao'] as $index => $observacao) {
        $observacao = trim($observacao); // Remove espaços em branco extras
...
<div class="itens-carrinho">
    ...
    if (!empty($_SESSION['carrinho'])) {
        foreach ($_SESSION['carrinho'] as $index => $item) {
            ...
            // Verifica se há uma observação e a exibe
